    ...
    public class PhotonLobby : MonoBehaviourPunCallbacks, ILobbyCallbacks
    {
            ...
            if (PhotonNetwork.IsConnected == false)
            {
                ...
                if (PlayerPrefs.HasKey("NickName"))
                {
                    PhotonNetwork.NickName = PlayerPrefs.GetString("NickName") + "-" +
                                             Random.Range(0, int.MaxValue).ToString("00000000");
                }
                else
                {
        ...
        public void CreateRoom()
        {
            ...
            RoomOptions roomOptions = new RoomOptions()
                {IsVisible = true, IsOpen = true, MaxPlayers = 16, CustomRoomProperties = h};
        ...
        public void CreateRoom(string name, bool visible, bool open, byte players, int map = 0)
        {
            ...
            h.Add("Map", map);
            RoomOptions roomOptions = new RoomOptions()
                {IsVisible = true, IsOpen = true, MaxPlayers = players, CustomRoomProperties = h};
